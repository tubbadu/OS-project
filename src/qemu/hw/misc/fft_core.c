...
// #define PI 3.14159265358979323846 // TODO find a better way
...
static void fft(complex double *, complex double *, int);
...
static void compute_fft(FFTCoreState *s, int size) {
	complex double invalues [SIZE];
	complex double outvalues[SIZE];
	
	for (int k=0; k<SIZE; k++){
		double real, imag;
		real = (double) s->input[2*k    ];
		imag = (double) s->input[2*k + 1];
		invalues[k] = real + imag * I;
	}

	fft(invalues, outvalues, SIZE);
	
	for (int k=0; k<SIZE; k++){
		s->output[2*k    ] = (uint64_t) creal(outvalues[k]);
...
void fft(complex double *invalues, complex double *outvalues, int N) {
	if (N <= 1) {
		if (N == 1) {
	...
	for (int i = 0; i < N / 2; i++) {
		even[i] = invalues[i * 2];
		odd[i] = invalues[i * 2 + 1];
	}
	
	// Conquer
	complex double *even_out = malloc(N / 2 * sizeof(complex double));
	complex double *odd_out = malloc(N / 2 * sizeof(complex double));
	fft(even, even_out, N / 2);
	fft(odd, odd_out, N / 2);
	
	// Combine
	for (int k = 0; k < N / 2; k++) {
		complex double t = cexp(-2.0 * I * M_PI * k / N) * odd_out[k];
		outvalues[k] = even_out[k] + t;
		outvalues[k + N / 2] = even_out[k] - t;
	}
